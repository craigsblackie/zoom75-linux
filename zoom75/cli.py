...
async def cmd_image(args):
    data = im.load_still(args.path, args.fit)
    screen = await _open(args)
    try:
...
async def cmd_gif(args):
    frames = im.load_frames(args.path, args.fit, max_frames=args.max_frames)
    if not frames:
        print("no frames in that file", file=sys.stderr)
        return 1
    blob = im.build_animation(frames, speed=args.speed)
